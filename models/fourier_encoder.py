...
class GeometryFourierEncoder():
    def __init__(self, args, device='cpu'):
        ...
        if args.sampling_strategy == 'fft':
            self.U, self.V = self.create_fft_meshgrid(args.fft_params)
            # reshape for batch operations
            self.U = self.U[None, :, :].to(device)
            self.V = self.V[None, :, :].to(device)
        elif args.sampling_strategy == 'gfm':
            self.U, self.V = self.create_gfm_meshgrid(args.gfm_params)
            # reshape for batch operations
            self.U = self.U[None, :, :].to(device)
            self.V = self.V[None, :, :].to(device)
        elif args.sampling_strategy == 'learned':
            max_freq = args.learned_f_params["max_freq"]
            min_freq = args.learned_f_params["min_freq"]
            self.U = nn.Parameter(min_freq + (max_freq - min_freq) * torch.rand(1, 21, 10)).to(device)
            self.V = nn.Parameter(max_freq * torch.rand(1, 21, 10)).to(device)
